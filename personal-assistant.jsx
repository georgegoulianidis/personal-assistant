...
export default function PersonalAssistant() {
    ...
    (async () => {
      try {
        const [t, n, c] = await Promise.all([
          window.storage.get(STORAGE_KEYS.tasks).catch(() => null),
          window.storage.get(STORAGE_KEYS.notes).catch(() => null),
          window.storage.get(STORAGE_KEYS.contacts).catch(() => null),
        ]);
  ...
  // Save helpers — return the promise so callers can await before navigating away
  ...
  const saveTasks = (next) => {
    setTasks(next);
    return persist(STORAGE_KEYS.tasks, next);
  };
  const saveNotes = (next) => {
    setNotes(next);
    return persist(STORAGE_KEYS.notes, next);
  };
  const saveContacts = (next) => {
    setContacts(next);
    return persist(STORAGE_KEYS.contacts, next);
  };
